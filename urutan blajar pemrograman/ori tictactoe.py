...
class TicTacToeGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Tic Tac Toe arpiani")
    ...
    def create_buttons(self):
        for i in range(3):
            for j in range(3):
                button = tk.Button(self.root, text='', font=('times', 40), width=5, height=2, bg='green', command=lambda row=i, col=j: self.on_button_click(row, col))
    ...
    def end_game(self):
        ...
        if winner_val:
            messagebox.showinfo("permainan selesai", f"pemenang: {winner_val}")
        else:
            messagebox.showinfo("permainan selesai", "game seri")
        self.reset_board()
